...
const SECRET =
  ...
  (() => {
    ...
    if (!IS_PROD) return 'deepsea-dev-secret';
...
const FROM = process.env.MAIL_FROM || 'Deep Sea <no-reply@deepsea.local>';

function subject(code: string) {
  return `${code} là mã đăng nhập Deep Sea của bạn`;
}

function body(code: string) {
  const text = `Mã đăng nhập Deep Sea của bạn: ${code}\n\nMã có hiệu lực trong 10 phút. Nếu bạn không yêu cầu mã này, cứ bỏ qua email.`;
  const html = `<div style="font-family:system-ui,sans-serif;max-width:420px;margin:auto;padding:24px;color:#1b1e24">
  <h2 style="margin:0 0 12px;font-size:18px">Mã đăng nhập Deep Sea</h2>
  <p style="margin:0 0 16px;color:#555">Nhập mã này vào trang game để vào chơi:</p>
  <div style="font-size:34px;font-weight:800;letter-spacing:10px;padding:14px 0;text-align:center;background:#e3f1f0;border-radius:12px">${code}</div>
...
/** "Deep Sea <me@gmail.com>" → { name: 'Deep Sea', email: 'me@gmail.com' } */
