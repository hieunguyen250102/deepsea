...
const SERVER_URL: string =
  ...
  (import.meta.env.DEV ? 'http://localhost:4100' : window.location.origin);

/** Email login: session + "code is on its way" state, shared with the other Oink games. */
export const authClient = createAuthClient({ storagePrefix: 'deepsea', serverUrl: SERVER_URL });
...
const KEY = 'deepsea.identity.v1';

export interface Identity {
  playerId: string;
  name: string;
  /** preferred diver colour; the table hands out another if it is taken */
  color: number;
}
...
export function loadIdentity(): Identity {
    ...
    if (raw) {
      ...
      if (parsed.playerId) {
        return {
          ...
          name: parsed.name ?? '',
          color: parsed.color ?? Math.floor(Math.random() * 6),
        };
      }
    }
  } catch {
  ...
  return { playerId: randomId(), name: '', color: Math.floor(Math.random() * 6) };
}
...
const ROOM_KEY = 'deepsea.room.v1';
